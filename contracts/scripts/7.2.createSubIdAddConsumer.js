...
const main = async () => {
    ...
    await writeConfig("0config","0config","SubId",txRep.events[0].args.subId.toString());
    
    let result = await isContractTransferSuccess(

        await VRFCoordinatorV2_5Contract.addConsumer(
            txRep.events[0].args.subId.toString(),
